...
export default function PromotionsPage() {
  ...
  const JOURS = [t('sunday'), t('monday'), t('tuesday'), t('wednesday'), t('thursday'), t('friday'), t('saturday')];
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-gray-800 border border-gray-700 rounded-lg p-4">
            <p className="text-gray-400 text-sm">{t('totalCodes')}</p>
            <p className="text-3xl font-bold">{promotions.length}</p>
          </div>
          <div className="bg-green-600/20 border border-green-600/50 rounded-lg p-4">
            <p className="text-green-400 text-sm">{t('activeCodes')}</p>
            <p className="text-3xl font-bold text-green-400">{activePromos.length}</p>
          </div>
          <div className="bg-blue-600/20 border border-blue-600/50 rounded-lg p-4">
            <p className="text-blue-400 text-sm">{t('totalUses')}</p>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className={`text-xs px-2 py-1 rounded ${
                        ...
                        {promo.status === 'ACTIVE' ? t('active') : t('inactive')}
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                      ...
                      <div>
                        <p className="text-gray-500">{t('uses')}</p>
                        <p className="font-semibold">
                          {promo.currentUses}
                  ...
                  <div className="flex flex-col gap-2">
                    <button
                      ...
                      title={promo.status === 'ACTIVE' ? t('deactivate') : t('activate')}
                    >
                      {promo.status === 'ACTIVE' ? (
                        <ToggleRight size={16} />
                      ) : (
          ...
          <div className="bg-gray-800 border border-gray-700 rounded-lg max-w-md w-full max-h-[90vh] overflow-y-auto">
            <div className="border-b border-gray-700 p-6 flex items-center justify-between sticky top-0 bg-gray-800">
              <h2 className="text-2xl font-bold">
                {editingPromo ? t('editTitle') : t('createTitle')}
              </h2>
              <button
                onClick={resetForm}
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                <label className="text-sm text-gray-400 block mb-2">{t('code')}</label>
                <input
                  type="text"
                  value={formData.code}
              ...
              <div>
                <label className="text-sm text-gray-400 block mb-2">{t('descriptionLabel')}</label>
                <textarea
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm text-gray-400 block mb-2">{t('type')}</label>
                  <select
                    value={formData.type}
                ...
                <div>
                  <label className="text-sm text-gray-400 block mb-2">{t('value')}</label>
                  <input
                    type="number"
                    step="0.01"
              ...
              <div>
                <label className="text-sm text-gray-400 block mb-2">{t('maxUses')}</label>
                <input
                  type="number"
                  min="1"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm text-gray-400 block mb-2">{t('startDateLabel')}</label>
                  <input
                    type="date"
                ...
                <div>
                  <label className="text-sm text-gray-400 block mb-2">{t('endDateLabel')}</label>
                  <input
                    type="date"
              ...
              <div>
                <label className="text-sm text-gray-400 block mb-2">
                  {t('timeRangeLabel')}
                  <span className="ml-1 text-gray-500">{t('timeRangeOptional')}</span>
                </label>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                ...
                {formData.activeFromTime && formData.activeToTime && (
                  <p className="mt-1 text-xs text-orange-400">
                    {t('timeRangeNote', { from: formData.activeFromTime, to: formData.activeToTime })}
                  </p>
                )}
              </div>
              ...
              <div>
                <label className="text-sm text-gray-400 block mb-2">
                  {t('activeDays')}
                  <span className="ml-1 text-gray-500">{t('activeDaysNote')}</span>
                </label>
